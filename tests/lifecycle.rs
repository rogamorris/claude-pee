...
if [ "${FAKE_DESCENDANT:-0}" = "1" ]; then (trap '' HUP TERM; sleep 30) & fi
...
fn invoke(mode: &str, descendant: bool, quiesce_ms: u64) -> (Output, String, Duration) {
    ...
    (output, records, elapsed)
}

#[test]
fn normal_hook_completion_returns_one_review_and_clean_outcome() {
    let (output, records, elapsed) = invoke("normal", false, 20);
    assert!(
...
fn missing_hook_forces_cleanup_and_preserves_completed_review() {
    let (output, records, elapsed) = invoke("missing-hook", false, 20);
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn silent_inference_is_bounded_and_returns_no_review() {
    let (output, records, elapsed) = invoke("no-output", false, 20);
...
fn pre_prompt_wait_is_cancelled_by_launch_relative_deadline() {
    let (output, records, elapsed) = invoke("never-quiescent", false, 2_000);
    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
    assert!(!records.contains("\"phase\":\"prompt_submitted\""));
    assert!(records.contains("\"outcome\":\"failed_incomplete\""));
...
fn surviving_descendant_is_killed_before_review_is_released() {
    let (output, records, elapsed) = invoke("normal", true, 20);
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn ignored_exit_is_forced_without_duplicating_review() {
    let (output, records, elapsed) = invoke("ignored-exit", false, 20);
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn child_exit_before_output_is_incomplete_and_content_free() {
    let (output, records, elapsed) = invoke("exit-early", false, 20);
    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
    assert!(records.contains("\"outcome\":\"failed_incomplete\""));
    assert!(elapsed < Duration::from_secs(3));
...
fn final_drain_recovers_terminal_output_written_immediately_before_exit() {
    let (output, records, elapsed) = invoke("exit-after-output", false, 20);
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
